<?php

use yii\grid\GridView;
use yii\helpers\Html;
use yii\helpers\Url;
use yii\widgets\ActiveForm;
use frontend\models\Comment;



/* @var $this yii\web\View */
/* @var $dataProvider yii\data\ActiveDataProvider */
/* @var $imageProvider yii\data\ActiveDataProvider */
/* @var $comment frontend\models\Comment */
/* @var $ticket frontend\models\Ticket */

$this->title = 'Chat';
$this->params['breadcrumbs'][] = ['label' => 'Tickets', 'url' => ['all-tickets']];
$this->params['breadcrumbs'][] = $this->title;
$comment= new Comment;

?>

<div class="ticket-index">
    <h1>
        <?= Html::encode('Title: '.$ticket->title) ?>
    </h1>
    <h1>
        <?= Html::encode('Status: '.($ticket->status ? 'Opened' : 'Closed')) ?>
    </h1>
    <?php $form = ActiveForm::begin(); ?>

    <p>
        <?= Html::a('Back', Url::previous(), ['class' => 'btn btn-success']); ?>
    </p>
    <?php ActiveForm::end();?>


    <?php
    if($ticket->admin_id == null){
        ?>
        <?php $form = ActiveForm::begin([
            'action' => '/ticket/administrate',
        ]); ?>
        <p>
            <?= Html::submitButton('Administrate', [
                'value' => $ticket->id,
                'name' => 'add',
                'class' => 'btn btn-danger pull-right',
                'data' => ['confirm' => 'Are you sure you want to administrate this ticket?']]); ?>
        </p>
        <?php
    } else {
        ?>
        <?php $form = ActiveForm::begin([
            'action' => '/ticket/close-ticket',
        ]); ?>
        <p>
            <?= Html::submitButton('Close status', [
                'value' => $ticket->id,
                'name' => 'close',
                'class' => 'btn btn-danger pull-right',
                'data' => ['confirm' => 'Are you sure you want to close this ticket?']]); ?>
        </p>

        <?php
    }
    ?>
    <?php ActiveForm::end();?>

    <?= GridView::widget([
        'dataProvider' => $dataProvider,
        'rowOptions' => function ($model, $key, $index) {
            if ($model['user_id'] == Yii::$app->user->id) {
                return $index % 2 ? ['style' => 'background-color: #d8e3d5' ] : ['style' => 'background-color: #ced9cb' ];

            } else {
                return $index % 2 ? ['style' => 'background-color: #f7f7f7' ] : ['style' => 'background-color: #f0f0f0' ];
            }
        },
        'columns' => [
            ['class' => 'yii\grid\SerialColumn'],
            [
                'attribute' => 'user_id',
                'value' => 'user.name',
                'label' => 'Name',

            ],
            'message',
            'create_time',
        ],
    ]); ?>

    <?php
    if($ticket->admin_id == Yii::$app->user->id){
        ?>
        <?php $form = ActiveForm::begin([
            'action' => '/ticket/send-message',

        ]); ?>
        <p>
            <?=  $form->field($comment, 'message')->textarea(['maxlength' => true]) ?>
        </p>

        <p>
            <?= Html::submitButton('Send message', ['value' => $ticket->id,'name' => 'ticketId', 'class' => 'btn btn-success']) ?>
        </p>
        <?php
        ActiveForm::end();
    }
    ?>
    <br>
    <h1>
        <?= Html::encode('Images:') ?>
    </h1>

    <?= Yii::$app->controller->renderPartial('_showImages',['imageProvider' => $imageProvider]);?>

</div>
